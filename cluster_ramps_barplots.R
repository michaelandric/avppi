...
for (ramp in ramps)
{
    ...
    for (i in 1:length(subjects))
    {
      ...
      subj_mat[, i] <- read.table(paste
                                  ('deconvolve_outs_ramps/',
                                    cn,'_',ramp,'_coef_',s_nums[i],'.txt', sep=''))$V1
    }
  ...
  for (ef in effects)
  {
    if (ef == 'Intxn' & ramp == 'rampup') next
    else{
      main_ef <- ef
      cl <- read.table(paste
                       ('ramp_effects/clust_',ef,'_',ramp,'_mema_out_mask.txt', sep=''))$V1
      cl_var_name <- paste('clust_',ef,'_',ramp,sep='')
  ...
  pdf(paste('cluster_',ramp,'_effects_plots2x2_bars_errbrs.pdf', sep=''))
