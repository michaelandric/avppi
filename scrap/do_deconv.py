...
def deconv(stdoutdir, input, seq, outpref):
    ...
    deconargs = split("3dDeconvolve -input %s -force_TR 1.5 -polort A \
...
#            infname = '%s_%s.Powered.cleanEPI.nii' % (ss, seq)
            infname = 'v8.%s_%s.Powered.cleanEPI.nii.gz' % (ss, seq)
#            infile = os.path.join(os.environ['avp'], 'nii',
#                                  '%s_CNR.anat' % ss, infname)
            infile = os.path.join(os.environ['avp'], 'nii',
                                  infname)
            decon_outdir = os.path.join(os.environ['avp'],
                                        'nii', 'deconvolve_outs_trilininterp')
            outpref = 'decon_out.block.%s_%s.Powered.cleanEPI' % (ss, seq)
