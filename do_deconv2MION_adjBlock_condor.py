...
This version makes it easier to adjust block duration.
...
def deconv(stdoutdir, inputset, seq, censorf, wmfile, ventfile, blk, outpref):
    """
    3dDeconvolve
    
    Block duration is a variable
    """
    ...
    deconargs = split("3dDeconvolve -jobs 2 -input %s \
                      ...
                      (inputset, censorf, sf_list[0], sf_list[1], model,
                       sf_list[2], model, sf_list[3], model, sf_list[4],
                       model, sf_list[5], wmfile, ventfile, 
...
if __name__ == '__main__':
    ...
    for bl in [10, 15, 20]:
        ...
        outpref = 'decon_out.mion.%s_concat.Powered.cleanEPI_%sblk' % (ss, bl)
        outfile = os.path.join(decon_outdir, outpref)
        deconv(decon_outdir, inputs, seq, cf, wm_file, vent_file, bl, outfile)
